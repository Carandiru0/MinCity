...
NK_API int
nk_sdf_hover_button(struct nk_context* const __restrict ctx,
	struct nk_image const* const __restrict imgSDF, bool const isActive = false, bool* const __restrict pbHovered = nullptr,
	struct nk_color color = nk_white) // optional colorize
{
	NK_ASSERT(ctx);
	NK_ASSERT(ctx->current);
	NK_ASSERT(ctx->current->layout);
	...
	if ((state = nk_widget(&r, ctx)))
	{
		...
		button = ctx->style.button;
		//button.normal = nk_style_item_color(nk_rgba(color.r, color.r, color.r, 127));
		//button.hover = nk_style_item_color(nk_rgba(color.r, color.r, color.r, 255));
		//button.active = nk_style_item_color(nk_rgba(255, 105, 180, 255));
		ret = nk_do_button(&ctx->last_widget_state, &win->buffer, r, 
			&button, in, ctx->button_behavior, &content);
		...
		if (isActive || (NK_WIDGET_STATE_ACTIVE == (NK_WIDGET_STATE_ACTIVE & ctx->last_widget_state)))
			color = nk_rgba(255, 105, 180, 255);
		else if (NK_WIDGET_STATE_HOVERED == (NK_WIDGET_STATE_HOVERED & ctx->last_widget_state)) {
			color = nk_rgba(color.r, color.r, color.r, 255);
			hovered = true;
		}
		else
			color = nk_rgba(color.r, color.r, color.r, 127);

		if (pbHovered) {
			*pbHovered = hovered;
		}

		nk_draw_image(cb, r, imgSDF, color);
	}
