...
namespace world
{
	...
	void __vectorcall cTestGameObject::OnVoxel(FXMVECTOR xmIndex, Volumetric::voxB::voxelDescPacked& __restrict voxel, void const* const __restrict _this, uint32_t const vxl_index)
	{
		reinterpret_cast<cTestGameObject const* const>(_this)->OnVoxel(xmIndex, voxel, vxl_index);
	}
	// ***** watchout - thread safety is a concern here this method is executed in parallel ******
	void __vectorcall cTestGameObject::OnVoxel(FXMVECTOR xmIndex, Volumetric::voxB::voxelDescPacked& __restrict voxel, uint32_t const vxl_index) const
	{
